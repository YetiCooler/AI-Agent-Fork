...
import { ProDisableIcon } from "@/components/assets/pro";
...
const InputBox = () => {
  ...
  useEffect(() => {
    if (!sessionId) {
      setSessionId(generateSessionId(profile?.email as string, Date.now().toString()));
    }
  }, [sessionId]);
  ...
  const generateResearch = async (prompt: string, requestSessionId: string) => {
    ...
    try {
      ...
      if (res.status == 429) {
        ...
        toast.error(`You've already used your ${profile?.currentplan.price == 0 ? Credits.free : Credits.pro} credits per month. Please try again after ${msg.availableInDays} days.`);
        return;
      }
      const data = await res.json();
