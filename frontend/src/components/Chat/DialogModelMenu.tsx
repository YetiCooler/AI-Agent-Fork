...
const DialogModelMenu = () => {
    ...
    return (
        <div>
            ...
            <div
                className={`fixed inset-0 bg-black/30 backdrop-blur-sm transition-opacity duration-500 z-50 ${isOpen ? "block" : "hidden"}`}
                onClick={() => setIsOpen(false)}
            >
            </div>
            <div className={`z-50 rounded-2xl flex flex-col w-[98vw] sm:w-[520px] h-[500px] fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#0C0C0E] border border-[#25252799] p-4 ${!isOpen && "hidden"}`}>
